...
function send_notifs(req, resp) {
    ...
    firebase.database().ref('PWA_users').once('value').then(function (data) {
        if (data.val() == null) {
            console.log('no data')
        } else {
...
const token = "EAAFspsZAMUr8BAP03QzmPnWBMikcVHhEOzZBIlTdTH0GtolCwHN88XXTXVeQosRGIfLtl60yvGQW2NSK0XHd2YWHqSzqLrra8aWeZAFLQG49sDXFSyWA9RhItwusnwnTrUeLNY52iZATD9zck7b0kXHfNejOZAJSVqIZBOkgXbXQZDZD";
...
app.get("/weather", function (req, res) {
    ...
    request(url, function (error, response, body) {
        ...
        let resp = Math.round(toto.hourly[2].temp - 273.15) + "° " + toto.hourly[2].weather[0].description + ";" + toto.hourly[2].weather[0].icon;
        res.send(resp);
    });
});
